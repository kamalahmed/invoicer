      ...
      <div className="grid grid-cols-1 gap-3 p-4 sm:grid-cols-2 lg:grid-cols-3">
        {INDUSTRY_PRESETS.map((p) => (
          <button
            ...
            className="group overflow-hidden rounded-xl border border-slate-200 bg-white text-left shadow-sm transition hover:border-slate-300 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-ink/30"
            title={p.description}
          >
            <div
            ...
            <div className="px-4 py-3">
              ...
              <div className="mt-1 text-xs text-ink-muted line-clamp-2">{p.description}</div>
              <div className="mt-2 text-xs font-medium text-ink-soft group-hover:text-ink">
                Start →
              </div>
            </div>
          </button>
