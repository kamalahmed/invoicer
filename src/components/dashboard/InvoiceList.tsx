  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
      <header className="flex items-center justify-between gap-2 border-b border-slate-200 px-5 py-3 dark:border-slate-800">
        <div>
          <h2 className="text-sm font-semibold text-ink dark:text-slate-100">Invoices</h2>
      ...
      {empty ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="text-left text-xs font-semibold uppercase tracking-wide text-ink-muted dark:text-slate-400">
              <tr>
                ...
                return (
                  ...
                  >
                    <td className="px-5 py-3 font-medium">
                      {inv.meta.number || '—'}
                      {isCurrent && (
                        <span className="ml-2 chip bg-emerald-100 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300">
                          current
                        </span>
                      )}
                    </td>
                    <td className="px-3 py-3">
                      ...
                      {inv.client.email && (
                        <div className="truncate text-xs text-ink-muted dark:text-slate-400">{inv.client.email}</div>
                      )}
                    </td>
                    <td className="px-3 py-3 text-ink-soft dark:text-slate-300">{formatDate(inv.meta.date)}</td>
